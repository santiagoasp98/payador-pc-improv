...
\begin{itemize}
    ...
    \item \textbf{Puzles con consecuencias}: Su resolución produce efectos tangibles modelados directamente en su definición (p. ej.: entregar objetos específicos, desbloquear pasajes concretos, o revelar información necesaria para el objetivo).
...
Este diseño orientado al objetivo (\textit{goal-oriented}, es decir, donde cada elemento del mundo está conectado lógica y funcionalmente al objetivo principal) tiene ventajas importantes:
\begin{itemize}
...
La Figura \ref{fig:world-class-diagram} muestra la estructura de clases completa y las relaciones entre los componentes del mundo. El sistema se compone de seis clases principales: \verb|World| (contenedor principal), \verb|Location| (espacios o lugares del mundo), \verb|Item| (objetos), \verb|Character| (personajes), \verb|Puzzle| (desafíos), y \verb|Objective| (meta del mundo). A continuación detallaremos cada una de las clases presentes en el sistema.
...
La clase \verb|World| constituye el contenedor principal de todos los elementos del juego y encapsula la filosofía de diseño fundamental: un mundo narrativo (\textit{story-driven}, guiado por la historia) orientado al objetivo (\textit{goal-oriented}) donde cada elemento está conectado lógica y semánticamente al objetivo principal. La clase \verb|World| encapsula los siguientes elementos estructurales:
\begin{itemize}
...
Estas validaciones aseguran que los NPCs no solo sean temáticamente apropiados sino también funcionalmente integrados en la mecánica del juego, permitiendo intercambios de objetos, proposición de puzles, y que el jugador avance hacia el objetivo de manera fluida y consistente con la narrativa.
...
Uno de los primeros cambios que implementamos fue la introducción de tipos explícitos de puzles. Sin esta categorización, el modelo de lenguaje quedaba sin guía clara al momento de crear desafíos, generando puzles inconsistentes o mal estructurados. Definimos siete tipos de puzles que el sistema puede generar, inspirados en taxonomías clásicas de puzles en narrativa interactiva y juegos de aventura, adaptadas a las capacidades del sistema. La Tabla \ref{tab:tipos_puzzles} presenta cada tipo con su descripción y un ejemplo representativo.
...
\begin{itemize}
    ...
    \item \textbf{proposed\_by\_character}: Nombre del personaje que propone el puzle, o vacío si el puzle es de descubrimiento ambiental
    \item \textbf{proposed\_by\_location}: Nombre de la ubicación cuya investigación revela el puzle (para puzles ambientales), vacío si es propuesto por un personaje
    \item \textbf{rewards}: Recompensa obtenida al resolver el puzle (p. ej.: objetos específicos, pasajes desbloqueados)
    \item \textbf{relevance\_to\_objective}: Explicación de cómo resolver este puzle contribuye a alcanzar el objetivo principal del mundo. Por ejemplo, si el objetivo es hablar con alguien y el puzle desbloquea una puerta, este campo diría ``Resolver este acertijo abrirá la puerta que lleva a la celda donde está el capitán''. Este campo ayuda al jugador a entender por qué vale la pena resolver el desafío
    \item \textbf{puzle\_hints}: Lista de pistas progresivas para ayudar al jugador. Las pistas siempre son tres y se entregan en orden de más general a más específica. Por ejemplo, para un acertijo sobre una papa: ``Crece bajo tierra'', ``Es un tubérculo comestible'', ``Se usa para hacer puré y papas fritas''
...
\begin{itemize}
    ...
    \item Se muestra al jugador un mensaje especial con el nombre de la pista, su descripción, su relevancia con respecto a resolver el objetivo de misterio, y el progreso actual (p. ej.: ``2 de 4 pistas descubiertas'')
    \item Al descubrir todas las pistas, el objetivo se completa y se revela el \verb|mystery_solution|
\end{itemize}

Al igual que hicimos para los puzles, también incluimos reglas especiales en el prompt (que se detallará en el anexo correspondiente, ver Sección \ref{anexo:prompts}) para favorecer que los objetivos de misterio se generen correctamente. Las reglas principales especifican que:

\begin{itemize}
    \item Cada pista debe estar asociada únicamente con un objeto físico que existe en el mundo, nunca con personajes o ubicaciones. El atributo \verb|associated_item| debe corresponder exactamente al valor del atributo \verb|name| de un objeto que aparece en la lista de objetos del mundo
    \item El objetivo debe especificar explícitamente tanto la lista \verb|mystery_clues| (pistas detalladas) como \verb|mystery_solution| (solución completa del misterio)
    \item En este caso no se usa el atributo \verb|completion_narration|; en su lugar se presenta la solución del misterio mediante \verb|mystery_solution|
\end{itemize}
...
Esta decisión se tomó porque crear objetos automáticamente para pistas podía resultar en objetos desconectados de la narrativa, mientras que eliminar pistas simplemente reducía el número de pistas a descubrir, manteniendo la coherencia del mundo. Esta diferencia en el enfoque de validación refleja las diferentes prioridades: los puzles son obstáculos activos que bloquean progresión y deben funcionar correctamente, mientras que las pistas son elementos opcionales de descubrimiento que pueden ajustarse en cantidad sin comprometer la jugabilidad fundamental.
...
La etapa final enriquece el mundo con contenido extra que incrementa la inmersión: ubicaciones secundarias que no son necesarias para completar el objetivo, personajes complementarios, objetos decorativos o de utilidad marginal, y rutas alternativas que permiten al jugador explorar caminos opcionales sin afectar el cumplimiento del objetivo principal.
...
El sistema aplica estrategias de corrección diferenciadas según el tipo de error detectado. Las recompensas faltantes en puzles se corrigen automáticamente durante el mismo intento de generación, mientras que cuando se detecta que un objetivo es imposible de completar, el sistema descarta completamente la generación de esa etapa y solicita una nueva al modelo de lenguaje. Esta asimetría refleja diferencias en criticidad: los puzles son problemas localizados que pueden repararse sin comprometer la estructura general del mundo, mientras que los objetivos imposibles invalidan completamente la jugabilidad. Los detalles de implementación de estas estrategias se presentan en la Sección \ref{sec:orquestacion_pipeline} del Capítulo \ref{cap:implementacion}.

Sin embargo, esta estrategia de rechazo para objetivos presenta una limitación fundamental: si el modelo es consistentemente inconsistente (generando repetidamente objetivos con objetos que no incluye en la lista de elementos del mundo), los tres reintentos fallarán y el pipeline completo fallará. La validación de completabilidad de objetivos detecta correctamente estos problemas, pero no puede corregirlos automáticamente. Más adelante, al detallar la clase Puzzle en el contexto de implementación, explicaremos cómo la verificación y corrección automática de recompensas de puzles sí logra subsanar estos errores, evidenciando esta diferencia en las capacidades de corrección según el tipo de error. El modelo puede omitir un objeto importante del objetivo en \verb|world.items| aunque lo referencie en \verb|objective.components|, y las instrucciones del prompt no siempre son suficientes para prevenir esta inconsistencia.
...
\begin{itemize}
    ...
    \item \textbf{Lógica de juego y ciclo de interacción}: El \textit{game loop} que combina generación narrativa del modelo con intervenciones deterministas del sistema simbólico.
...
La integración con modelos de lenguaje es uno de los desafíos centrales del enfoque neurosimbólico, y en particular porque IVIE presenta requisitos que no satisface PAYADOR, que fue construido lo más minimal posible para ser expandido. Para este trabajo utilizamos principalmente Gemini 2.0 Flash de Google y GPT-4o-mini de OpenAI como modelos de lenguaje base. Necesitamos que el modelo de lenguaje genere mundos jugables, pero la salida en lenguaje natural no garantiza la coherencia estructural requerida por el motor del juego, lo cual implica una dificultad técnica significativa. Resolvemos esto usando los modelos de lenguaje junto a la biblioteca Pydantic\footnote{Pydantic es una biblioteca de Python para validación de datos usando anotaciones de tipo. Define ``modelos'' que son clases Python con campos tipados y reglas de validación. Ver: \url{https://docs.pydantic.dev/latest/concepts/models/}}, para que su salida respete una estructura determinada.
...
\begin{itemize}
    \item \textbf{Etapa 1} (Concepto del mundo): Genera título, \textit{backstory}, concepto del jugador y objetivo
...
El sistema aplica estrategias de corrección diferenciadas según el tipo de error detectado. Cuando un puzle tiene recompensas faltantes (es decir, cuando el modelo omitió especificar qué recompensa otorga el puzle al ser resuelto), el sistema puede corregir automáticamente este problema durante el mismo intento de generación mediante la función \verb|verify_puzzle_rewards_and_fix()|, sin necesidad de regenerar toda la etapa. Esta corrección automática permite aprovechar generaciones que presentan errores estructurales menores fácilmente subsanables. Por el contrario, cuando se detecta que un objetivo es imposible de completar (por ejemplo, el modelo especificó conseguir un objeto que no incluyó en la lista de elementos del mundo), el sistema descarta completamente la generación de esa etapa y solicita una nueva generación al modelo de lenguaje. Esta asimetría en el manejo de errores tiene su origen en la naturaleza de cada problema: cuando un puzle presenta un error estructural, el sistema puede corregirlo localmente; en cambio, cuando un objetivo es imposible de completar, el mundo completo se vuelve injugable y debe regenerarse.

Sin embargo, esta estrategia de rechazo para objetivos presenta una limitación fundamental: si el modelo es consistentemente inconsistente (generando repetidamente objetivos con objetos que no incluye en la lista de elementos del mundo), los tres reintentos fallarán y el pipeline completo terminará en falla. La validación de completabilidad de objetivos detecta correctamente estos problemas, pero no puede corregirlos automáticamente como sí hace la verificación y corrección automática de recompensas de puzles mediante \verb|verify_puzzle_rewards_and_fix()|. Esta asimetría refleja una diferencia fundamental en las capacidades de corrección: mientras que los puzles con recompensas faltantes pueden subsanarse creando los objetos necesarios (como se describió en el Capítulo \ref{cap:diseno}), los objetivos imposibles requieren regeneración completa de la etapa. El modelo puede omitir un objeto importante del objetivo en \verb|world.items| aunque lo referencie en \verb|objective.components|, y las instrucciones del prompt no siempre son suficientes para prevenir esta inconsistencia.

Como se discutirá en el Capítulo \ref{cap:evaluacion}, esta fragilidad resultó en algunos mundos que, tras agotar los reintentos, pasaron las validaciones pero presentaron problemas sutiles durante el juego, evidenciando que la validación post-generación no puede compensar completamente la inconsistencia inherente del modelo durante la generación.
...
Tras la Etapa 4, se verifica tamaño del mundo, conectividad y completabilidad del objetivo mediante validaciones finales. Si alguna de estas tres validaciones finales (tamaño, conectividad, completabilidad) falla, se emiten advertencias pero no se rechaza el mundo porque ya se alcanzó el límite de intentos. En la práctica, estas validaciones finales rara vez fallan si las validaciones intermedias de cada etapa fueron exitosas, ya que los problemas críticos habrán sido detectados y corregidos en etapas anteriores.

\section{Lógica de Juego y Ciclo de Interacción} \label{sec:logica_juego}

Es importante explicar por qué el ciclo de interacción (\textit{game loop}) es necesario antes de describirlo. Un ciclo de juego coordina la secuencia repetitiva de acciones que constituyen la experiencia del jugador: recibir entrada, procesarla, actualizar el estado del mundo, generar retroalimentación narrativa y verificar condiciones de victoria o derrota. Sin este ciclo, el sistema no podría mantener coherencia entre turnos ni gestionar la progresión del jugador hacia el objetivo.
...
Los adaptadores de modelos (\verb|GeminiModel|, \verb|OpenAIModel|) implementan manejo robusto de errores de red. Dada esta facilidad dispuesta por los adaptadores, pudimos detectar automáticamente aquellos casos en que los servicios de la API retornan errores de tipo 503 (servicio temporalmente sobrecargado), aplicando reintentos automáticos con una espera fija de 2 segundos entre cada intento. En el caso de la API de OpenAI, implementamos detección específica de limitaciones de tasa (\textit{rate limiting}, error 429), que indican que se ha excedido el límite de solicitudes por minuto. Cuando se detecta este error, el sistema aplica espera exponencial (\textit{exponential backoff}): la espera se duplica en cada reintento.

\verb|prompt_model_structured()| en \verb|GeminiModel| maneja casos especiales de JSON malformado mediante verificaciones específicas. En primer lugar, si la respuesta incluye marcadores markdown (\verb|```json|), los remueve automáticamente. Además, implementamos \verb|_is_json_complete()| que valida heurísticamente la completitud del JSON mediante tres verificaciones secuenciales: (1) verificar que la respuesta no esté vacía, (2) verificar que la cantidad de llaves de apertura coincida con la cantidad de llaves de cierre, y (3) verificar que la cantidad de corchetes de apertura coincida con la cantidad de corchetes de cierre. Cuando el sistema detecta que el JSON retornado está incompleto, se reintenta la generación completa con el mismo prompt hasta alcanzar un máximo de 3 intentos.
...
Las estrategias de prompting y el manejo robusto de errores presentadas en esta sección permiten que el sistema interactúe de forma confiable con los modelos de lenguaje, garantizando que las salidas generadas sean estructuralmente válidas y que el sistema pueda recuperarse de fallos transitorios de la API o errores de formato en las respuestas. A continuación describiremos cómo los jugadores interactúan con este sistema mediante la interfaz de usuario.
...
El modo \texttt{inspiration} permite al usuario proporcionar una frase o temática que inspira la generación del mundo, como definimos en la Sección \ref{sec:pipeline} del capítulo anterior. Implementamos un campo de texto donde el usuario ingresa conceptos (p. ej.: ``el misterio sobre el robo del cuadro'' o ``exploración espacial en una estación abandonada''). Esta entrada se pasa directamente como parámetro \verb|theme| a \verb|run_step_1_concept()|, como se explicó en el Capítulo \ref{cap:diseno}.
...
Cuando generamos el mundo desde un texto de inspiración (correspondiente al modo \texttt{inspiration} detallado en la Sección \ref{sec:modos_juego}), el sistema ejecuta las cuatro etapas del pipeline de forma secuencial con indicaciones visuales del progreso. Cada etapa muestra su mensaje de progreso (p. ej.: ``Etapa 1: Generando concepto del mundo...''), actualiza una barra de progreso y muestra mensajes de confirmación con detalles. Este \textit{feedback} permite informarle al usuario sobre el proceso que se está ejecutando, durante los 30-60 segundos que típicamente toma la generación completa.

\subsubsection{Modo Generate}

El modo \texttt{generate} crea mundos completamente aleatorios sin entrada del usuario, como se describió en la Sección \ref{sec:pipeline}. Este modo solicita al modelo proponer un tema original y luego procede con el pipeline completo. Observamos durante el desarrollo que este modo produce mundos variados e inesperados, aunque el conjunto de conceptos que el modelo maneja es relativamente limitado, porque notamos repetición de conceptos, personajes o lugares. Probablemente esto esté relacionado con el caché que el modelo mantiene (información que el modelo reutiliza entre solicitudes para optimizar rendimiento). Como se alinea con la limitación que tienen los modelos al generar narrativa con temáticas diferentes \cite{garcia2024payador, ammanabrolu2024letter}, esta falta de variación temática es un problema conocido en el campo.

El botón único ``Generate Random World'' reduce las barreras de entrada para el usuario, ya que no requiere proporcionar ninguna entrada inicial. Esta simplicidad resulta especialmente útil durante el desarrollo, permitiendo iteraciones rápidas de prueba sin necesidad de configuración previa.
...
En etapas tempranas del desarrollo usamos Gradio como framework de UI (mencionado brevemente en la sección de trabajos relacionados). Sin embargo, migramos a Streamlit porque ofrece mejor control sobre el estado de sesión, mayor flexibilidad en el layout de componentes, y una API más natural para aplicaciones conversacionales complejas.

Streamlit usa un modelo de ejecución stateless donde cada interacción re-ejecuta el script completo. Manejamos estado persistente mediante \verb|st.session_state|, un diccionario que sobrevive entre ejecuciones. Almacenamos allí el objeto \verb|World|, el \verb|game_loop|, el historial de chat, ubicaciones visitadas, configuración de idioma y modo de generación (p. ej.: \verb|st.session_state.world = None|).

Ya que cada interacción puede modificar el estado y necesitamos prevenir sobrescritura accidental de datos existentes, implementamos inicialización condicional al inicio del script: verificamos la existencia de cada clave en \verb|session_state| antes de asignar valores por defecto. Esta inicialización lazy (evaluación diferida que solo inicializa variables cuando es necesario) previene resetear el estado cuando el usuario interactúa con otros widgets (los widgets son los componentes interactivos de la interfaz como botones, campos de texto y selectores).
...
Generamos visualizaciones del mundo como diagramas de grafo usando sintaxis Mermaid. Un módulo especializado (\verb|world_visualizer.py|) exporta el mundo a formato Mermaid\footnote{\url{https://mermaid.js.org/}}, que es una biblioteca JavaScript para crear diagramas a partir de definiciones textuales. Esta representación es importante porque permite inspeccionar rápidamente la estructura del mundo generado, identificando problemas de conectividad o validación sin necesidad de navegar toda la narrativa. La función \verb|render_mermaid()| en la UI lo renderiza mediante una página HTML embebida con Mermaid.js desde CDN. Esta visualización permite al usuario (o a nosotros durante el desarrollo) ver la estructura completa del mundo como un grafo navegable con nodos que representan ubicaciones y aristas que representan conexiones entre ellas. Incluimos un screenshot de esta funcionalidad en la Figura \ref{fig:mermaid_viz}, que ilustra cómo el concepto neurosimbólico permite tanto generar contenido mediante el modelo como validar y visualizar su estructura mediante el sistema simbólico.
...
Implementamos un sidebar persistente con \verb|st.sidebar| que contiene controles de configuración global (p. ej.: selector de idioma español/inglés, selector de modo de generación, campo de identificación del jugador, opción de modo de depuración).Bloqueamos estos controles durante juego activo para prevenir cambios de configuración que corrompan el estado del mundo a mitad de partida.

La identificación del jugador (\textit{nickname}) que se puede ingresar en el sidebar permite identificar a los evaluadores durante las pruebas de usuario, facilitando el análisis posterior de las trazas guardadas. Sin embargo, creemos que esta funcionalidad podría tener otros usos a futuro, como por ejemplo personalizar la narrativa según preferencias del jugador o analizar patrones de juego específicos por usuario.

\subsection{Experiencia de Usuario Multilingüe}

Soportar múltiples idiomas no es solo traducir strings de la interfaz, sino garantizar coherencia narrativa completa en cada idioma sin mezcla de lenguajes. La coherencia narrativa significa que si el jugador elige español, toda la experiencia debe estar en español: los textos de la interfaz, las narraciones generadas por el modelo, los nombres de personajes y ubicaciones, y los mensajes del sistema. Una mezcla accidental (p. ej.: interfaz en español pero narración en inglés) rompe la inmersión y confunde al usuario. Centralizamos todos los textos de UI en \verb|ui_components.py| mediante funciones que retornan diccionarios de strings. Cada string tiene una clave invariante y valores específicos por idioma. Este patrón centralizado es útil y simple, ya que permite detectar strings faltantes cuando se agrega un nuevo idioma (las claves sin traducción generan errores visibles). Sin embargo, dificulta agregar nuevos idiomas, pues requiere extender manualmente todos los diccionarios de strings.

Para garantizar coherencia en la generación narrativa, el parámetro \verb|language| se propaga a través de toda la cadena de generación: desde la UI hasta los prompts del modelo. Los prompts tienen versiones paralelas en español e inglés, y funciones dispatcher (funciones que seleccionan dinámicamente qué versión del prompt usar según el idioma de la partida actual) eligen la versión correcta. Esto asegura que el modelo reciba instrucciones en el mismo idioma que la partida actual y genere narraciones en español o en inglés según corresponda.

Finalmente, el sistema RAG mantiene consistencia de idioma almacenando el parámetro \verb|language| en metadatos de trazas MongoDB. Al cargar memorias con \verb|load_memories_from_db()|, usamos el idioma original de la traza para regenerar resúmenes.

En experimentos preliminares observamos que existe un desafío relevante: mantener la sincronización entre versiones de idiomas. La sincronización se refiere a mantener paridad entre las versiones en español y en inglés de los prompts y textos de interfaz. Por ejemplo, si agregamos un objeto a la versión en español de un prompt, debemos recordar agregarlo a la versión en inglés. Implementamos validaciones básicas que verifican estructura paralela, aunque no pueden verificar equivalencia semántica completa de contenidos. Es importante aclarar que el pipeline se ejecuta solamente para el idioma elegido por el usuario (español XOR inglés), no generamos el mismo mundo en ambos idiomas. La sincronización se refiere únicamente al mantenimiento del código fuente: cuando modificamos un prompt en español, debemos aplicar el cambio equivalente en la versión en inglés para mantener funcionalidad consistente en ambos idiomas.
